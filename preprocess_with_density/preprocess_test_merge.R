...
zipcode <- read_csv("data/zipcode_combin_test.csv")
...
df_test$host_since <- as.Date(df_test$host_since, origin="1960-10-01",  tryFormats = c("%m/%d/%Y"))
...
library(stringr)
for (i in c(1:length(df_test$amenities))){
  am1 <- gsub("[^A-Za-z0-9]", ",", df_test$amenities[i])
  df_test$new_amenities[i] <- str_replace(am1, "\\,", "")
  # df$new_amenities[i] <- str_replace_all(ame2, "\\?", "") 
}


## Internet/Wifi/Wireless
df_test$Internet <- ifelse(str_detect(tolower(df_test$new_amenities),"internet|wifi|wireless")==TRUE, 1, 0)
...
df_test$Cooking <- ifelse(str_detect(tolower(df_test$new_amenities),"kitchen|dishes|cooking")==TRUE, 1, 0)
df_test$Cooking <- impute(df_test$Cooking, roundup =  TRUE) # impute 2NAs with mode from test data


## Detector
df_test$Detector <- ifelse(str_detect(tolower(df_test$new_amenities),"smoke|extinguisher|detector")==TRUE, 1, 0)
df_test$Detector <- impute(df_test$Detector, roundup =  TRUE) # impute 2NAs with mode from test data

## Smoking Allowed
df_test$SmokingAllowed <- ifelse(str_detect(tolower(df_test$new_amenities),"smoking")==TRUE, 1, 0)
df_test$SmokingAllowed <- impute(df_test$SmokingAllowed, roundup =  TRUE) # impute 2NAs with mode from test data

## Essentials
df_test$Essentials <- ifelse(str_detect(tolower(df_test$new_amenities),"essentials")==TRUE,1,0)
df_test$Essentials <- impute(df_test$Essentials, roundup =  TRUE) # impute 2NAs with mode from test data


## Lock
df_test$Lock <- ifelse(str_detect(tolower(df_test$new_amenities),"lock|lockbox")==TRUE,1,0)
df_test$Lock <- impute(df_test$Lock, roundup =  TRUE) # impute 2NAs with mode from test data


## Parking
df_test$Parking <- ifelse(str_detect(tolower(df_test$new_amenities),"parking")==TRUE,1,0)
df_test$Parking <- impute(df_test$Parking, roundup =  TRUE) # impute 2NAs with mode from test data


## Air_conditioning
df_test$Air_conditioning <- ifelse(str_detect(tolower(df_test$new_amenities),"conditioning")==TRUE,1,0)
df_test$Air_conditioning <- impute(df_test$Air_conditioning, roundup =  TRUE) # impute 2NAs with mode from test data


## Wokspace
df_test$Workspace <- ifelse(str_detect(tolower(df_test$new_amenities),"workspace")==TRUE,1,0)
df_test$Workspace <- impute(df_test$Workspace, roundup =  TRUE) # impute 2NAs with mode from test data


## Elevator
df_test$Elevator <- ifelse(str_detect(tolower(df_test$new_amenities),"elevator")==TRUE,1,0)
df_test$Elevator  <- impute(df_test$Elevator , roundup =  TRUE) # impute 2NAs with mode from test data


## Gym
df_test$Gym <- ifelse(str_detect(tolower(df_test$new_amenities),"gym")==TRUE,1,0)
df_test$Gym <- impute(df_test$Gym, roundup =  TRUE) # impute 2NAs with mode from test data


## Refrigerator
df_test$Refrigerator <- ifelse(str_detect(tolower(df_test$new_amenities),"refrigerator")==TRUE,1,0)
df_test$Refrigerator <- impute(df_test$Refrigerator, roundup =  TRUE) # impute 2NAs with mode from test data


## Stove
df_test$Stove <- ifelse(str_detect(tolower(df_test$new_amenities),"stove")==TRUE,1,0)
df_test$Stove <- impute(df_test$Stove, roundup =  TRUE) # impute 2NAs with mode from test data

## Check
df_test$Check <- ifelse(str_detect(tolower(df_test$new_amenities),"24|check")==TRUE,1,0)
df_test$Check<- impute(df_test$Check, roundup =  TRUE) # impute 2NAs with mode from test data

## Family
df_test$Family <- ifelse(str_detect(tolower(df_test$new_amenities),"kid|family")==TRUE,1,0)
df_test$Family <- impute(df_test$Family, roundup =  TRUE) # impute 2NAs with mode from test data

selected_test <- add_test('Family')
selected_test <- add_test('Check')
selected_test <- add_test('Stove')
selected_test <- add_test('Refrigerator')
selected_test <- add_test('Gym')
selected_test <- add_test('Elevator')
selected_test <- add_test('Workspace')
selected_test <- add_test('Air_conditioning')
selected_test <- add_test('Parking')
selected_test <- add_test('Lock')
selected_test <- add_test('Essentials')
selected_test <- add_test('Detector')
selected_test <- add_test('SmokingAllowed')
selected_test <- add_test('Cooking')
...
write.csv(export, file="data/test_cleaned8.csv", row.names = FALSE) #Write dataframe as CSV
