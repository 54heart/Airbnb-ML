...
zipcode <- read_csv("data/zipcode_combin_train.csv")
...
selected <- c(selected, selected_zip)
print(selected)
length(selected)
...
# convert data type
export_train[,48:66] <- lapply(export_train[,48:66], factor) #"monthly_suitable"~ "interaction"  to factor
...
write.csv(export_train, file="data/train_cleaned8.csv", row.names = FALSE) #Write dataframe as CSV
