...
sum(is.na(df$high_booking_rate)) # There are 19 NA values
selectedNA <- addNA("high_booking_rate")

# var2 accommodates---------------
### Remove 9 bad recoreds
check(df$accommodates) # 9 NA values -> so we add this column to NA list
...
# 4 availablity_30--------------
check(df$availability_30) # 10 NA values -> so add it to selectedNA() list
...
check(df$availability_365) # 9 NA values so add it to selected
selectedNA <-addNA('availability_365')

# var6 availability_60
df$availability_60 <- df$availability_60 - df$availability_30
check(df$availability_60) # 10 NA values -> add to selectedNA
...
check(df$availability_90) # 10 NA values , so add to selectedNA
selectedNA <-addNA('availability_90')

# var8 bathrooms---------------

check(df$bathrooms) #254 NA values, so we add to selectedNA list
...
check(df$Real_Bed) # 1 NA value, so we add to selectedNA list
selectedNA <- addNA('Real_Bed')

# var10 bedrooms---------------
## impute 92 NA with mean, and roundup to no decimals
check(df$bedrooms) # 101 NA values, so we add to selectedNA list
selectedNA <- addNA('bedrooms')

# 11 beds---------------
check(df$beds) #83 NA values, so add to selectedNA
...
check(df$cancellation_policy) # 0 NA values, so add to selected list
...
check(df$cleaning_fee) # 18334 NA values, so add to selectedNA list
...
check(df$first_review) # 0 NA values, scale the values
...
check(df$host_has_profile_pic) # 142 NA values, so add to selectedNA list
...
check(df$host_identity_verified) #142 NA values, so add to selectedNA list
...
check(df$host_is_superhost) #142 NA values, so add to selectedNA list
selectedNA <- addNA('host_is_superhost')


# 28 host_listings_count---------------
check(df$host_listings_count)#142 NA values, add to selectedNA
...
check(df$host_response_rate) #15793 NA values, add to selectedNA
...
check(df$host_response_time) # 15793 na values, add to selectedNA list
...
check(df$experience)
df$experience <- scale(df$experience) #142 NA
...
check(df$instant_bookable) # 10 NA values, add to selectedNA
...
check(df$is_business_travel_ready) #44548 NA values, add to selectedNA
...
check(df$is_location_exact) # 10 NA values so add to selectedNA
...
check(df$long_stay) #10 NA values, add to selectedNA
selectedNA <- addNA('long_stay')


# 48 minimum_nights-----------------
check(df$minimum_nights) # 10 NA values, add to selectedNA
...
check(df$require_guest_phone_verification) # 10 NA values, add to selectedNA list
...
check(df$require_guest_profile_picture) # 10 NA values, add to selectedNA list
selectedNA <- addNA('require_guest_profile_picture')

# 58 requires_license---------------
df$requires_license <- ifelse(df$requires_license==TRUE, 1, 0)
check(df$requires_license) # 10 NA values, add to selectedNA
...
check(df$security_deposit) #44276 NA values, add to selectedNA
...
#df_selectedNA$experience <- round(df_selectedNA$experience,digit = 2)
df_selectedNA$price <- round(df_selectedNA$price,digit = 2)
...
nrow(df_selected) == nrow(df_selectednonNA) # TRUE

df_final_train <- cbind(df_selected,df_selectednonNA)

#View(df_final_train)
write.csv(df_final_train, file="train_cleaned(mice).csv", row.names = FALSE) #Write dataframe as CSV
